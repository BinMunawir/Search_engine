...
public class BST<T extends Comparable<? super T>> {
    ...
    public Queue getNodes(){
    	Queue<T> q = new Queue<>();
    	getNode(root,q);
    	return q;
    }
    private void getNode(BSTNode<T> p, Queue<T> q) {
    	if(p!=null){
    		getNode(p.right,q);
    		q.enqueue(p.el);
    		getNode(p.left,q);
    ...
    public T searchByComparing(T el){
    	BSTNode<T> p = root;
    	while(p!=null){
    		int c = el.compareTo(p.el);
    		if(c<0)
    			p=p.left;
    		else if (c>0)
    			p=p.right;
    ...
    private int length(BSTNode<T> p) {
		if(p!=null){
			return 1+length(p.left)+length(p.right);
		}
		return 0;
	}

	protected String print(BSTNode<T> p,String s) {
        if (p != null) {
        	String ss="{"+p.el+"} , ";
        	return print(p.left,s) +ss+ print(p.right,s);
        }
        return "";
    }
